...
全家共用一組密碼，擋在 Cloudflare Worker 的 `/api/chat`、`/api/tts` 前面。
...
- **不保護 Supabase**：前端用 `VITE_SUPABASE_ANON_KEY` 直連 Supabase，
  這把 key 本來就設計成可以在前端公開（CLAUDE.md 第 10 節）。
  有心人只要看過前端原始碼拿到 anon key，還是能直接打 Supabase 的
  `profiles`／`tasks`／`errors` 等表，這道密碼完全擋不住。
  這是刻意的取捨（Option A 輕量方案），不是漏掉沒做。
