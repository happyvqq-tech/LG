...
function wordRegexFor(language: Language): RegExp {
  // 日文沒有分詞空白，只能靠字元類型抓可能的「一個詞」：漢字詞（含々）
  // 跟片假名詞都是可靠的邊界——片假名幾乎只用來寫外來語/擬聲詞/專有名詞，
  // 一段連續片假名基本上就是一個詞（跟漢字詞同理）。平假名做不到這件事：
  // 沒有斷詞的話一段連續平假名常常是好幾個詞黏在一起（動詞+語尾+助詞…），
  // 硬挖出來會把整句挖掉一大半，比「太短」的訊息還糟，所以刻意不挖平假名。
  if (language === '日文') return /[一-鿿々ァ-ヶー]+/g
